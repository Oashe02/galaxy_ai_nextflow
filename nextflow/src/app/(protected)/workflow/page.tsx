...
import { Search, Play, History, Layers, Settings, User } from 'lucide-react';
import { useReactFlow } from '@xyflow/react';

const NODE_TYPES = [
  { name: 'Text Node', icon: Layers, type: 'text' },
  { name: 'Upload Image', icon: Layers, type: 'uploadImage' },
  { name: 'Upload Video', icon: Layers, type: 'uploadVideo' },
  { name: 'Run Any LLM', icon: Play, type: 'llm' },
  { name: 'Crop Image', icon: Layers, type: 'cropImage' },
  { name: 'Extract Frame', icon: Layers, type: 'extractFrame' },
];
...
export default function WorkflowPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-black text-white selection:bg-purple-500/30">
      ...
      <aside className="w-64 border-r border-zinc-900 flex flex-col bg-[#050505]">
                ...
                {NODE_TYPES.map((node) => (
                  ...
                  >
                    <div className="w-8 h-8 rounded bg-zinc-900 border border-zinc-800 flex items-center justify-center group-hover:bg-zinc-800 transition-colors">
                      <node.icon className="w-4 h-4 text-zinc-400 group-hover:text-purple-400" />
                    </div>
                    {node.name}
                  </button>
                ))}
        ...
        <div className="mt-auto p-4 border-t border-zinc-900 text-[10px] text-zinc-600 font-medium">
      ...
      <main className="flex-1 relative flex flex-col">
        ...
        <div className="h-12 border-b border-zinc-900 bg-[#050505] flex items-center justify-between px-4 z-10">
          ...
          <div className="flex items-center gap-3">
            <button className="px-4 py-1.5 bg-purple-600 hover:bg-purple-500 text-white rounded-full text-xs font-bold transition-all flex items-center gap-2 shadow-lg shadow-purple-500/10">
      ...
      <aside className="w-72 border-l border-zinc-900 flex flex-col bg-[#050505]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div className="flex flex-col items-center justify-center py-20 text-center opacity-40">
            ...
            <p className="text-xs max-w-[160px] mt-1 text-zinc-600">Run your workflow to see execution history here.</p>
          </div>
        </div>
      </aside>
    </div>
